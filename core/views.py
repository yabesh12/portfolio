...
def home(request):
	try:
		...
		if person.exists():
			...
			# Get SEO projects
			seo_projects_objs = ProjectDetail.objects.filter(user=person_obj, project_category__iexact="SEO")
			print(seo_projects_objs)
			# Get Web projects
			web_projects_objs = ProjectDetail.objects.filter(user=person_obj, project_category__iexact="WEB")

			# Get other projects
			other_projects_objs = ProjectDetail.objects.filter(user=person_obj, project_category__iexact="others")
...
def project_details(request, slug):
	...
	try:
	 	project_image_objs = ProjectDetailImage.objects.prefetch_related('project').filter(project__name__iexact=slug)
...
class EmailThread(threading.Thread):
	...
	def run(self):
		msg = EmailMessage(self.subject, self.message, EMAIL_HOST_USER, self.to)
		print("mail sent")
		msg.send()
...
def ajax_send_email_message(request):
	if request.is_ajax:
		...
		msg = "Message sent Successfully!. I'll contact you ASAP."
		print(msg)
